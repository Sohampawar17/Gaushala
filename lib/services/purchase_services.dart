...
class PurchaseService {
  ...
  Future<PurchaseMaster?> masters() async {
    try {
      final url =
          '$baseurl/api/method/goshala_sanpra.custom_pyfile.login_master.purchaseMasters';
      ...
      if (response.statusCode == 200) {
        return PurchaseMaster.fromJson(response.data["data"]);
      } else {
  ...
  Future<bool> delete(PurchaseList customer) async {
    try {
      final url = '$baseurl/api/resource/Purchase Receipt/${customer.name}';
  ...
  Future<bool> create(Collection customer) async {
    try {
      final url = '$baseurl/api/resource/Purchase Receipt';
