...
import 'package:gaushala/screens/sales/add_sales/add_sales_viewmodel.dart';
...
class SelectCustomerScreen extends StatelessWidget {
  ...
  Future<SalesMaster> _fetchSuppliers() async {
    final service = SalesService();
    return await service.masters() ?? SalesMaster();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<SalesMaster>(
        ...
        builder: (context, snapshot) {
          ...
          final suppliers = master.customer ?? [];
...
class _SalesFormScreenState extends State<SalesFormScreen> {
  void _addItem(Items item, AddSalesViewModel model) {
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  content: SizedBox(
                                    ...
                                    child: ListView.builder(
                                      shrinkWrap: true,
                                      itemCount:
                                          purchase.salesItems?.length ?? 0,
                                      itemBuilder: (context, i) {
                                        final item = purchase.salesItems![i];
                                        return Card(
                                          elevation: 2,
                                          margin: const EdgeInsets.symmetric(
                                              vertical: 4),
